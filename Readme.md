...
<hr>
The normal architecture or the http work on sending a request to the server and getting the data from the server with either of modifying the server resources using the POST method or getting the server resources by hiting with GET method. The problem with this architecture is that anyone can act as a proxy and can get the data that server sent to you and it's not secure at all.

The https introduced with enhanced sucurity. And thats how SSL certificates work along. 
...
**Symmetric:**
If there is single key that can encrypt as well decrypt the data then it's a symmetric encryption. The issue with this encryption is if I am a client for the encrypted data i have a key so I need to send that key to server when server got my encrypted data to just decrypt it. the key can be hacked by middle man and what ever you communicate in future hacker can decrypt.

So we can introduce some secure transfer of key...

**Asymmetric enCryption:**

in Asymmetric encryption we have a public key that encrypt the data and the private key that only can decrypt the data.

**How do then two system transfer encrypted data securely:**

In case of client server architecture. the server has its own public and private key as  well the client also have his public and private key. Now first server send his public key to the client (remember: client is the one who is goin to request resources to server). client received the public key of server. now client will generate the symmetric key and will encrypt it with servers public key. Now client sent his encrypted key to the server if the hacker hacked it still he can not open it because to open/decrypt that encrypted key he need private key and private key has not shared over network by server, so only server can decrypt the key that was encrypted by client (using public key of server). Now both the client and server has the same keys (the key that generated by client).There is also a possibility that it can be hacked.

How???
Hacker can act as a proxy. when the server is sending his public key to client the hacker can act as proxy and in middle he can change that servers public key to his own public key and then that hackers key has been sent to the client. Client has zero idea about the public key and it assume that the public key that he received is server's only. So here we fucked up. You can thought of next possible scenarios where the hacker can actually get the data which the client is sharing with the server because it has his key. 

**So here SSL certificates introduced!!!!**
The server has his own private and public key, by using those keys the server will generate the certificate, it ask lets encrypt (issuer) create a certificate for the server and lets encrypt also has its own public key. The certificate contain the domain name, the servers public key as well in signature section it has servers public key + lets encrypt public key creates a signature. The letsencrypt will share this certificate to the server now. When we want to communicate the server will share his public key to the client but client as well as server is not sure whether the public key is valid public key of server or not. So server will send the public key as well as certificate and tell client to varify the public key using certificate. Client will hit/reach the lets encrypt and ask his own public key and lets encrypt share his public key to client. Now client has public key of server and public key of the letsencrypt now with this combination the client can verify the signature of certificate. So if signature varified then client can trust the server and can share the data to the server over a network.
